#!/bin/bash
# =============================================================================
# tests/run_spx_batch.sh - Batch driver for SLH-DSA SPHINCS+ simulations
#
# Runs every SPHINCS+ variant (128f/192f/256f x robust/simple) for the
# requested phases (keygen, sign, verify) and snapshots each simulation log
# under a timestamped results directory.
#
# Usage (from anywhere):
#     bash tests/run_spx_batch.sh [options] PHASE [PHASE ...]
#
# Phases (positional, any subset, in any order):
#     keygen    -> TEST_KEY=1  TEST_SIGN=0  TEST_SIGN_OPEN=0
#     sign      -> TEST_KEY=0  TEST_SIGN=1  TEST_SIGN_OPEN=0
#     verify    -> TEST_KEY=0  TEST_SIGN=0  TEST_SIGN_OPEN=1
#     all       -> shorthand for "keygen sign verify"
#
# Options:
#     --mode {baseline|optimized|all}  Which implementation set to run.
#                                      Default: baseline.
#     --variants "list"                Space-separated list of variant names
#                                      to run instead of the default 6
#                                      (e.g. "SPHINCS-128f-robust SPHINCS-256f-simple").
#     --results-dir DIR                Output directory for collected logs.
#                                      Default: <repo>/tests/results/spx_batch_<timestamp>
#     -h | --help                      Show this help.
#
# Examples:
#     # All 18 baseline simulations (6 variants x keygen/sign/verify)
#     bash tests/run_spx_batch.sh --mode baseline keygen sign verify
#
#     # Just keygen for the 6 optimized variants
#     bash tests/run_spx_batch.sh --mode optimized keygen
#
#     # All 36 (baseline + optimized) with shorthand
#     bash tests/run_spx_batch.sh --mode all all
#
# Notes:
#   * The cva6 sim places its log at:
#         verif/sim/out_<YYYY-MM-DD>/veri-testharness_sim/main.<TARGET>.log.iss
#     The <YYYY-MM-DD> reflects the simulation start date, which may roll over
#     between runs. This script captures the start time of each simulation
#     and locates the matching log under any out_*/ directory whose log file
#     was modified after that timestamp, so multi-day batches still work.
# =============================================================================
...
_usage() {
    sed -n '2,/^# ===/p' "${BASH_SOURCE[0]}" | sed 's/^# \{0,1\}//' | head -n 60
}
...
while (( $# > 0 )); do
    case "$1" in
        -h|--help)
            _usage; exit 0 ;;
        --mode)
            [[ $# -ge 2 ]] || _die "--mode requires an argument"
            MODE="$2"; shift 2 ;;
        --mode=*)
            MODE="${1#*=}"; shift ;;
        --variants)
            [[ $# -ge 2 ]] || _die "--variants requires an argument"
            read -r -a VARIANTS <<<"$2"; shift 2 ;;
        --variants=*)
            read -r -a VARIANTS <<<"${1#*=}"; shift ;;
        --results-dir)
            [[ $# -ge 2 ]] || _die "--results-dir requires an argument"
        ...
        keygen|sign|verify)
            PHASES+=("$1"); shift ;;
        all)
            PHASES+=(keygen sign verify); shift ;;
        --) shift; break ;;
        -*)
...
[[ -z "$RESULTS_DIR" ]] && RESULTS_DIR="$TESTS_DIR/results/spx_batch_$TS"
...
# --- build the cva6.py command for one (mode, variant) ---------------------
# Mirrors the logic in tests/pqc/<mode>/DS/SLH-DSA/<variant>/run.sh, but
# injects the per-phase TEST_KEY/TEST_SIGN/TEST_SIGN_OPEN defines.
run_one() {
    local mode="$1" variant="$2" phase="$3"
    local tag="${mode}-${variant}-${phase}"
    ...
    # Variant-specific thash file (robust vs simple)
    local thash_file
    if [[ "$variant" == *-robust ]]; then
        thash_file="thash_shake_robust.c"
    else
        thash_file="thash_shake_simple.c"
    fi

    # Resolve SRC_BASE (some layouts use src/ subdir)
    ...
    if [[ ! -f ./verif/sim/setup-env.sh ]]; then
        echo "  ! missing ./verif/sim/setup-env.sh"; return 2
    fi
    ...
    # Marker file to detect the newly-produced log even across day rollovers.
    local marker; marker="$(mktemp -p "$SIM_DIR" .spx_batch_marker.XXXX)" || marker=""
    ...
    # Locate the produced .log.iss across any out_*/ dated directory.
    # Prefer files newer than $started; fall back to newest overall.
    local found=""
...
echo
echo "╔════════════════════════════════════════════════════════╗"
echo "║         SLH-DSA SPHINCS+ Batch Simulation Driver       ║"
echo "╚════════════════════════════════════════════════════════╝"
echo "Modes    : ${MODES[*]}"
