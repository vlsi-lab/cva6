...
{
    asm inline (
      ".insn r 0x7b, 0x1, 0x7, %[res0], %[a0], %[b0]\n\t"  
      ".insn r 0x7b, 0x1, 0x8, %[res1], x0, x0\n\t"  
...
int main() {
    ...
    for (int i = 0; i < NUM_TESTS; i++) {
        if (out[i][0] != out_ref[i].o[0] && out[i][1] != out_ref[i].o[1]) {
            printf("Test %d: Expected out[%d] = %"PRIu64"-%"PRIu64", but got %"PRIu64"-%"PRIu64"\n",
