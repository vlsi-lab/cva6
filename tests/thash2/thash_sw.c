/*
 * Software reference implementation of thash (tweakable hash)
 * for SPHINCS+ shake256-robust variant with 2-block input.
 * 
...
/* ========================================================================
 * thash implementation (robust variant)
 * ======================================================================== */
...
/*
 * thash_sw: Software reference implementation
 * 
 * Robust variant construction:
...
void thash_sw(uint8_t *out, const uint8_t *in, unsigned int inblocks,
    ...
    /* Buffer: pub_seed (16) + addr (32) + max input (2*16=32) = 80 bytes */
    uint8_t buf[SPX_N + SPX_ADDR_BYTES + THASH2_INPUT_BYTES];
    uint8_t bitmask[THASH2_INPUT_BYTES];
    size_t i;
    ...
    /* Step 1: Generate bitmask */
    shake256(bitmask, inlen, buf, SPX_N + SPX_ADDR_BYTES);

    /* Step 2: XOR input with bitmask, append to buffer */
    for (i = 0; i < inlen; i++) {
        buf[SPX_N + SPX_ADDR_BYTES + i] = in[i] ^ bitmask[i];
    }

    /* Step 3: Generate output */
    shake256(out, SPX_N, buf, SPX_N + SPX_ADDR_BYTES + inlen);
}
