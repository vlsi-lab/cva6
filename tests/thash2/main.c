...
static int run_thash2_case(const thash_variant_t *v, uint32_t *sw_cycles, uint32_t *hw_cycles, uint32_t seed) {
    uint8_t pub_seed[32] = {0};
    uint8_t addr[SPX_ADDR_BYTES] = {0};
    uint8_t in[64] = {0};
    uint8_t out_sw[32] = {0};
    uint8_t out_hw[32] = {0};
    uint8_t funct7 = spx_thash2_funct7_for_n(v->n);
    uint32_t c_sw = 0;
    uint32_t c_hw = 0;
    size_t inlen = 2 * v->n;
    ...
    spx_hw_exec(out_hw, pub_seed, addr, in, inlen, v->n, funct7, v->simple_mode);
...
int main(void) {
    ...
    for (size_t i = 0; i < sizeof(k_variants) / sizeof(k_variants[0]); i++) {
        for (uint32_t tc = 0; tc < 2; tc++) {
            ...
            printf("%s thash2 case%u: %s", k_variants[i].name, tc + 1, err ? "FAIL" : "PASS");
#if SW_TEST_ENABLED
