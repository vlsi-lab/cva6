...
/*************************************************
* Name:        shake256_finalize_domain
*
* Description: Finalize absorb step with an explicit domain-separation byte.
*              Added on top of the original ml-kem-512 fips202.c to support
*              cSHAKE256/KMAC256, whose domain separator (0x04) differs from
*              plain SHAKE's (0x1F).
*
* Arguments:   - keccak_state *state: pointer to Keccak state
*              - uint8_t domain: domain-separation byte
**************************************************/
void shake256_finalize_domain(keccak_state *state, uint8_t domain)
{
  keccak_finalize(state->s, state->pos, SHAKE256_RATE, domain);
  state->pos = SHAKE256_RATE;
}
