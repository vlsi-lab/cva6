...
/* Upstream selects the parameter set purely via a -DMAYO_VARIANT=MAYO_n
 * compile flag (see MAYO-C's src/CMakeLists.txt). For a compile-time-fixed
 * variant directory (no external -D flags needed, matching every other
 * Additional-DS family here), each MAYO_<N>/include/mayo_variant.h supplies
 * MAYO_VARIANT and MAYO_BUILD_TYPE_OPT before this header needs them. */
#ifndef MAYO_VARIANT
#include "mayo_variant.h"
#endif
