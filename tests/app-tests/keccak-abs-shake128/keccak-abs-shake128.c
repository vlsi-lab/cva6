/*
 * tests/app-tests/keccak-abs-shake128/keccak-abs-shake128.c
 *
 * Standalone SW-vs-HW microbenchmark: non-incremental SHAKE128 XOF
 * (absorb-once + squeeze, 168-byte rate), the primitive ML-DSA's matrix-A
 * expansion (poly_uniform(), tests/pqc/baseline/ML-DSA-44/poly.c) needs.
 * Same structure as the sibling keccak-abs-shake256 test, adapted for
 * SHAKE128's rate and JOBCTRL.RATE168=1 (see keccak_dma_ctrl.sv's Phase 1
 * rate generalization).
 *
 * SW reference: same public-domain Keccak-f[1600] core as
 * keccak-abs-shake256, just absorbed/squeezed at the 168-byte SHAKE128
 * rate instead of SHAKE256's 136 bytes.
 *
 * HW path: vrf_ip's job-based DMA absorb engine
 * (vrf_ip/rtl/keccak_dma_ctrl.sv, JOB_SRC_ADDR/JOB_SRC_LEN/
 * JOBCTRL.GO|FRESH|RATE168), plus the legacy CSREG raw-permute path for
 * squeeze blocks beyond the first -- same call sequence as
 * keccak-abs-shake256, with RATE168 set and a 168-byte squeeze/pad window.
 */
...
/* ===================================================================== */
/* SW reference: public-domain Keccak-f[1600] + SHAKE128 sponge.         */
/* ===================================================================== */
...
static void shake128(uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen)
{
  ...
  keccak_absorb_once(s, SHAKE128_RATE, in, inlen, 0x1F);
  keccak_squeeze(out, outlen, s, SHAKE128_RATE, SHAKE128_RATE);
}

/* ===================================================================== */
/* HW path: vrf_ip's job-based DMA absorb engine (RATE168=1) + legacy    */
/* CSREG raw-permute squeeze.                                            */
/* ===================================================================== */

#define VRF_HW_RATE 168
#define VRF_HW_PADDED_MAX 336  /* up to 2 rate blocks for this test's inputs */
...
keccak_hw_absorb_job(const void *in, uint32_t len, int fresh)
{
	...
	*job_src_len  = len;
	...
	ctrl |= (uint64_t)1 << VRF_JOBCTRL_RATE168_BIT;
	*jobctrl = ctrl;
 ...
 * job (RATE168=1), then squeeze outlen bytes, permuting between rate
 * blocks via the legacy CSREG raw-permute path.
 */
static void
...
static void
shake128_hw(uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen)
{
...
main(void)
{
	static uint8_t msg[200]; /* > 168-byte rate, exercises the multi-block absorb path too */
	static uint8_t sw_out[336], hw_out[336]; /* > 168 bytes, exercises the multi-block squeeze path too */
	uint32_t cycles_sw, cycles_hw;
	int i, mismatch;

	print_uart("=== keccak-abs-shake128: non-incremental SHAKE128 (rate=168) ===\n");
	...
	write_csr(mcycle, 0);
	shake128(sw_out, sizeof sw_out, msg, sizeof msg);
	...
	write_csr(mcycle, 0);
	shake128_hw(hw_out, sizeof hw_out, msg, sizeof msg);
	...
	print_uart("[FAIL] SHAKE128 SW/HW mismatch\n");
	print_uart("  SW: ");
	for (i = 0; i < 64; i ++) print_uart_byte(sw_out[i]);
	print_uart("\n  HW: ");
	for (i = 0; i < 64; i ++) print_uart_byte(hw_out[i]);
	print_uart("\n");
	return 1;
}
