/*
 * tests/app-tests/keccak-abs-multi-squeeze/keccak-abs-multi-squeeze.c
 *
 * Standalone SW-vs-HW microbenchmark: non-incremental XOF squeeze
 * spanning multiple rate blocks (a 50-byte message, 500-byte squeezed
 * output -- 500/136 = 4 output blocks), mirroring
 * hash-spx/cva6/tests/keccak-abs-multi-squeeze/main.c's SW-reference
 * logic and message/output sizes, but with the hardware call replaced by
 * digsig's vrf_ip AXI peripheral (base 0x5000_0000,
 * vrf_ip/sw/vrf_axi.h) instead of hash-spx's own CVXIF coprocessor.
 *
 * DEVIATION FROM THE REFERENCE TEST (explicit, see also run.sh): the
 * hash-spx reference exercises SHAKE128 (rate 168 bytes). vrf_ip's
 * DMA absorb engine (vrf_ip/rtl/keccak_dma_ctrl.sv) hardcodes
 * "RateBytes = 136" -- SHAKE256/SHA3-256's rate, the only rate this IP
 * targets (see that file's own header comment) -- with no runtime rate
 * parameter, so SHAKE128 cannot be offloaded to this hardware without an
 * RTL change, which is out of scope here. This test therefore exercises
 * SHAKE256 instead (both SW reference and HW path compute the same
 * function, so the SW-vs-HW comparison is still apples-to-apples); what
 * is preserved from the reference test is the *shape* of the exercise:
 * a squeeze output much longer than one rate block, forcing multiple
 * chained permutations on the output side.
 *
 * SW reference: fips202.c's keccak_absorb_once()/keccak_squeeze()/
 * KeccakF1600_StatePermute(), extracted from
 * hash-spx/cva6/tests/keccak-abs-multi-squeeze/fips202.c (public-domain
 * Keccak reference), trimmed to the SHAKE256-only, non-incremental
 * shake256() entry point this test exercises.
 *
 ...
 * plus the legacy CSREG raw-permute path (CSREG.START/DONE) to advance
 * the sponge between each of the 4 squeezed output blocks -- the actual
 * multi-squeeze exercise this test is named for. The pad10*1 padding
 * (message || 0x1F || 0x00...0x00 || final byte |= 0x80) is built in
 * software and absorbed as plain data instead of using JOBCTRL.FLIP (see
 * the sibling keccak-abs-shake256 test for why).
 */
...
static void shake256(uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen)
{
  ...
  keccak_absorb_once(s, SHAKE256_RATE, in, inlen, 0x1F);
  keccak_squeeze(out, outlen, s, SHAKE256_RATE, SHAKE256_RATE);
}
...
static void
shake256_hw(uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen)
{
	keccak_hw_hash(in, inlen, 0x1F, out, outlen);
}
...
main(void)
{
	...
	static uint8_t sw_out[500], hw_out[500];
	uint32_t cycles_sw, cycles_hw;
	int i, mismatch, sw_nonzero, hw_nonzero;

	print_uart("=== keccak-abs-multi-squeeze: SHAKE256, 500-byte squeeze ===\n");
	...
	sw_nonzero = 0;
	hw_nonzero = 0;
	for (i = 0; i < (int)sizeof sw_out; i ++) {
		if (sw_out[i] != 0) sw_nonzero = 1;
		if (hw_out[i] != 0) hw_nonzero = 1;
	}
	mismatch = memcmp(sw_out, hw_out, sizeof sw_out) != 0;
	...
	if (!sw_nonzero) {
		print_uart("[FAIL] SW squeeze output is all zeros\n");
		return 1;
	}
	if (!hw_nonzero) {
		print_uart("[FAIL] HW squeeze output is all zeros\n");
		return 1;
	}

	if (!mismatch) {
		print_uart("[PASS] HW matches SW element-wise (");
		print_uart_dec((int)(unsigned)sizeof sw_out);
		print_uart(" bytes, ");
		print_uart_dec((int)(unsigned)((sizeof sw_out + 135) / 136));
		print_uart(" squeeze blocks)\n");
		return 0;
	}

	print_uart("[FAIL] SHAKE256 multi-squeeze SW/HW mismatch\n");
	print_uart("  SW[0:32): ");
	for (i = 0; i < 32; i ++) print_uart_byte(sw_out[i]);
	print_uart("\n  HW[0:32): ");
	for (i = 0; i < 32; i ++) print_uart_byte(hw_out[i]);
	print_uart("\n");
	return 1;
}
