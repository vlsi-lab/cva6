/*
 * tests/app-tests/keccak-abs-sha3-256-single-block/keccak-abs-sha3-256-single-block.c
 *
 * Standalone SW-vs-HW microbenchmark: non-incremental SHA3-256
 * (absorb-once + squeeze), single rate-block message, mirroring
 * hash-spx/cva6/tests/keccak-abs-sha3-256-single-block/main.c's
 * SW-reference logic and NIST KAT (SHA3-256("abc")), but with the
 * hardware call replaced by digsig's vrf_ip AXI peripheral (base
 * 0x5000_0000, vrf_ip/sw/vrf_axi.h) instead of hash-spx's own
 * CVXIF coprocessor.
 *
 * SW reference: fips202.c's keccak_absorb_once()/keccak_squeeze()/
 * KeccakF1600_StatePermute(), extracted from
 * hash-spx/cva6/tests/keccak-abs-sha3-256-single-block/fips202.c
 ...
 * first. The pad10*1 padding (message || 0x06 || 0x00...0x00 || final
 ...
 * 0x1F (see keccak_dma_ctrl.sv's FLIP_BYTE0 state), correct for SHAKE256
 * (see the sibling keccak-abs-shake256 test) but wrong for SHA3-256's
 * 0x06 domain separator, so this test builds the padded block in
 * software and absorbs it as plain data instead.
 */
...
/*
 * NIST FIPS 202 KAT: SHA3-256("abc") =
 *   3a985da7 4fe225b2 045c172d 6bd390bd
 *   855f086e 3e9d525b 46bfe245 11431532
 */
static const uint8_t msg_abc[3] = { 0x61, 0x62, 0x63 };
...
main(void)
{
	...
	int sw_kat_ok, hw_kat_ok, sw_hw_match;

	print_uart("=== keccak-abs-sha3-256-single-block: SHA3-256(\"abc\") ===\n");

	clear_csr(mcountinhibit, 1);

	write_csr(mcycle, 0);
	sha3_256(sw_out, msg_abc, sizeof msg_abc);
	...
	write_csr(mcycle, 0);
	sha3_256_hw(hw_out, msg_abc, sizeof msg_abc);
	cycles_hw = (uint32_t)read_csr(mcycle);
	print_uart("HW cycles: ");
	print_uart_dec((int)cycles_hw);
	print_uart("\n");

	sw_kat_ok = memcmp(sw_out, kat_abc, 32) == 0;
	hw_kat_ok = memcmp(hw_out, kat_abc, 32) == 0;
	...
	if (sw_kat_ok && hw_kat_ok && sw_hw_match) {
		print_uart("[PASS] SW == HW == NIST KAT\n");
		return 0;
	}

	print_uart("[FAIL] SHA3-256(\"abc\") mismatch\n");
	print_uart("  Expected: "); print_hex32(kat_abc);
