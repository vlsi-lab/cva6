/*
 * tests/app-tests/keccak-abs-shake256/keccak-abs-shake256.c
 *
 * Standalone SW-vs-HW microbenchmark: non-incremental SHAKE256 XOF
 * (absorb-once + squeeze), mirroring
 * hash-spx/cva6/tests/keccak-abs-shake256/main.c's SW-reference logic
 * and test vector (a 100-byte counting message, 64-byte squeeze), but
 * with the hardware call replaced by digsig's vrf_ip AXI peripheral
 * (base 0x5000_0000, vrf_ip/sw/vrf_axi.h) instead of hash-spx's
 * own CVXIF coprocessor.
 *
 * SW reference: fips202.c's keccak_absorb_once()/keccak_squeeze()/
 * KeccakF1600_StatePermute(), extracted from
 * hash-spx/cva6/tests/keccak-abs-shake256/fips202.c (public-domain
 ...
 * plus the legacy CSREG raw-permute path for squeeze blocks beyond the
 * first. The pad10*1 padding (message || 0x1F || 0x00...0x00 || final
 * byte |= 0x80) is built in software and absorbed as plain data instead
 * of using JOBCTRL.FLIP: the DMA engine hardwires FLIP's domain byte to
 * 0x1F (see keccak_dma_ctrl.sv's FLIP_BYTE0 state), which happens to be
 * correct for SHAKE256 but not for SHA3-256 (0x06) -- the sibling
 * keccak-abs-sha3-256-* tests reuse this exact same helper with a
 * different domain byte, so building the padding once in software keeps
 * all four keccak-abs-* tests' hardware call sequence identical.
 */
...
main(void)
{
	...
	static uint8_t sw_out[64], hw_out[64];
	uint32_t cycles_sw, cycles_hw;
	int i, mismatch;

	print_uart("=== keccak-abs-shake256: non-incremental SHAKE256 ===\n");
	...
	print_uart("[FAIL] SHAKE256 SW/HW mismatch\n");
	print_uart("  SW: ");
