// TPA benchmark - Keccak-f[1600] permutation, configured for SHAKE256 (r = 1088 bits).
//
...
//                              from tests/keccak64/keccak_noopt.c)
//   - USE_COPROCESSOR_AXI:    "optimized" AXI accelerator IP call (verbatim
//                              from tests/keccak64/keccak_axi.c)
//
// Either way this measures C_perm: the clock-cycle cost of a single call to
// the full-state permutation. C_perm is a property of the Keccak-f[1600]
// permutation itself, not of the sponge rate -- so this is the same
// permutation used by tests/TPA/keccak-shake128, differing only in the rate
// label used downstream to compute TPA = r * f_clk / (C_perm * A).
...
static void KeccakF1600_StatePermute(uint64_t *s)
{
    ...
    for (int i = 0; i < 25; i++) cryptoState[i] = s[i];
    ...
    *csreg &= ~(1ULL << KECCAK_CSREG_START_BIT);
    ...
    for (int i = 0; i < 25; i++) s[i] = cryptoState[i];
}
...
static void KeccakF1600_StatePermute(uint64_t *s)
{
    ...
    s00 = s[0];  s01 = s[1];  s02 = s[2];  s03 = s[3];  s04 = s[4];
    s05 = s[5];  s06 = s[6];  s07 = s[7];  s08 = s[8];  s09 = s[9];
    s10 = s[10]; s11 = s[11]; s12 = s[12]; s13 = s[13]; s14 = s[14];
    s15 = s[15]; s16 = s[16]; s17 = s[17]; s18 = s[18]; s19 = s[19];
    s20 = s[20]; s21 = s[21]; s22 = s[22]; s23 = s[23]; s24 = s[24];
    ...
    for (round = 0; round < 24; round++) {
        ...
        s00 = s00 ^ C2; s05 = s05 ^ C2; s10 = s10 ^ C2; s15 = s15 ^ C2; s20 = s20 ^ C2;
        ...
        s01 = s01 ^ C2; s06 = s06 ^ C2; s11 = s11 ^ C2; s16 = s16 ^ C2; s21 = s21 ^ C2;
        ...
        s04 = s04 ^ C0; s09 = s09 ^ C0; s14 = s14 ^ C0; s19 = s19 ^ C0; s24 = s24 ^ C0;

        s03 = s03 ^ C3; s08 = s08 ^ C3; s13 = s13 ^ C3; s18 = s18 ^ C3; s23 = s23 ^ C3;

        s02 = s02 ^ C2; s07 = s07 ^ C2; s12 = s12 ^ C2; s17 = s17 ^ C2; s22 = s22 ^ C2;

        C1  = s05;
        s05 = ROL64(s03, 28); s03 = ROL64(s18, 21); s18 = ROL64(s17, 15); s17 = ROL64(s11, 10);
        s11 = ROL64(s07,  6); s07 = ROL64(s10,  3); s10 = ROL64(s01,  1); s01 = ROL64(s06, 44);
        s06 = ROL64(s09, 20); s09 = ROL64(s22, 61); s22 = ROL64(s14, 39); s14 = ROL64(s20, 18);
        s20 = ROL64(s02, 62); s02 = ROL64(s12, 43); s12 = ROL64(s13, 25); s13 = ROL64(s19,  8);
        s19 = ROL64(s23, 56); s23 = ROL64(s15, 41); s15 = ROL64(s04, 27); s04 = ROL64(s24, 14);
        s24 = ROL64(s21,  2); s21 = ROL64(s08, 55); s08 = ROL64(s16, 45); s16 = ROL64(C1,  36);
...
int main(void)
{
    ...
    printf("TPA benchmark - Keccak-f[1600] %s - %s (r=%d bits)\n", IMPL_NAME, ALGO_NAME, RATE_BITS);
