...
/*
 * Cycle accounting for the mp_NTT() dispatcher below, same rationale and
 * pattern as ng_mp31.c's ntt_dispatch_cycles -- answers "how much of
 * Verify is mp_NTT-call time" directly, rather than inferring it from
 * before/after KAT deltas. Printed by main.c right after "Verify OK".
 */
uint64_t vrfy_ntt_dispatch_cycles = 0;
uint64_t vrfy_ntt_dispatch_calls  = 0;

/*
 * Cycle accounting for the fx32_FFT()/fx32_iFFT() calls in verify_inner()
 * below -- added to measure the actual FFT-family share of Verify with
 * real numbers, prompted by the question of whether ntt_engine.sv's
 * Revision 3 (vect_FFT/vect_iFFT, ng_fxp.c) should also serve Verify.
 * It cannot without separate hardware: fx32_FFT/fx32_iFFT operate on a
 * DIFFERENT fixed-point format (Q1.31, uint32_t, scaled by 2^31, table
 * FX32_GM) than vect_FFT/vect_iFFT (Q32.32, fxr, scaled by 2^32, table
 * GM_TAB) -- different word width, different twiddle table, and a
 * different butterfly formula (fx32_FFT's forward pass halves every
 * stage; vect_FFT's does not). See NTT_ACCEL_DESIGN.md. This accounting
 * exists purely to measure the opportunity, not because hardware is
 * wired in here.
 */
uint64_t vrfy_fx32fft_dispatch_cycles = 0;
uint64_t vrfy_fx32fft_dispatch_calls  = 0;
...
/*
 * Moved up from this file's original location (just above GM_p1[]/GM_p2[])
 * so mp_NTT_sw()/GM_p1[]/GM_p2[] etc. below can still see them -- P1/P2
 * are no longer needed by mp_NTT_hw()/mp_NTT() themselves (ntt_engine.sv
 * now takes p/p0i as plain per-job values, see mp_NTT_hw()'s comment
 * below), but everything else in this file still keys off these names.
 */
...
/*
 * Hardware offload of mp_NTT() to ntt_engine.sv (keccak_ip/rtl/ntt_engine.sv).
 * ntt_engine.sv takes p/p0i as plain per-job register values (NTT_P_VAL/
 * NTT_P0I_VAL) rather than a fixed prime pair, so this works for whatever
 * modulus mp_NTT() is called with -- in this file that is always P1 or P2
 * (see the file header comment: "here, we will use them for exactly two
 * moduli"), but the hardware path itself no longer needs to know that.
 * Only the plain (non-auto-adjoint) NTT is redirected: mp_NTT_autoadj()
 * below exploits auto-adjoint conjugate symmetry with a completely
 * different index pattern (a[u]/a[hn-u], not a[k1]/a[k2] with k2=k1+ht,
 * and half as many butterflies per stage) that ntt_engine.sv does not
 * implement, so it stays software-only.
 *
 * `a` is an ordinary cached CPU buffer (carved out of the caller's tmp
 * buffer, e.g. `c1`/`c2` in vrfy_ntt_norm), and the peripheral writes
 * results back to DRAM as a second AXI master -- same class of
 * cache-coherency hazard sig_gauss_hw_job() (hawk_sign.c) already solved,
 * and NOT one a plain `fence` resolves on this SoC config, since
 * DcacheFlushOnFence/DcacheInvalidateOnFlush are both 0
 * (cv64a6_imac_crypto_config_pkg.sv). Staged through a fixed non-cacheable
 * DRAM scratch window instead, same convention as GAUSS_HW_SCRATCH_ADDR
 * (hawk_sign.c) but at a distinct address so the two never alias, even
 * though KeyGen/Sign/Verify never run concurrently in this KAT flow.
 *
 * `gm` itself needs no such treatment: GM_p1[]/GM_p2[] below are
 * compile-time-constant tables, never written at runtime, so their DRAM
 * contents are always correct regardless of what the CPU's D$ holds.
 */
#define KECCAK_AXI_BASE_ADDR 0x50000000UL
#define NTT_HW_SCRATCH_ADDR  0x80F01000UL
#define NTT_HW_SCRATCH_SIZE  4096u   /* n*4, n<=1024 (HAWK-1024) */
#define NTT_HW_MIN_LOGN      2u      /* below this, dispatch overhead > savings */

static void
mp_NTT_hw(unsigned logn, uint32_t *restrict a,
	const uint32_t *restrict gm, uint32_t p, uint32_t p0i, unsigned mode)
{
	...
	*ntt_p0i_val  = p0i;
	...
	/* MODE: 0=forward NTT, 2=mp_NTT_autoadj's reduced-butterfly phase
	   (see ntt_engine.sv's header comment) */
...
TARGET_AVX2
static void
mp_NTT_sw(unsigned logn, uint32_t *restrict a,
...
/*
 * Transparent dispatcher: every existing call site keeps calling mp_NTT()
 * unchanged (mp_poly_to_NTT() below, in turn called from vrfy_ntt_norm()).
 * Hardware is only reachable from the non-AVX2 build (HAWK_AVX2 targets
 * x86, not this RISC-V/CVA6 SoC); ntt_engine.sv now handles any modulus,
 * so the only remaining guard is the NTT_HW_MIN_LOGN size threshold.
 */
static void
...
#if !HAWK_AVX2
	if (logn >= NTT_HW_MIN_LOGN) {
		mp_NTT_hw(logn, a, gm, p, p0i, 0);
		vrfy_ntt_dispatch_cycles += read_csr(mcycle) - c0;
		vrfy_ntt_dispatch_calls  += 1;
		return;
	}
#endif
	mp_NTT_sw(logn, a, gm, p, p0i);
	vrfy_ntt_dispatch_cycles += read_csr(mcycle) - c0;
	vrfy_ntt_dispatch_calls  += 1;
}
 ...
 * Assumption: logn >= 3
 *
 * External linkage would not be needed here (only mp_NTT_autoadj() below
 * calls this), but kept static since nothing outside this file needs it.
 */
TARGET_AVX2
static void
mp_NTT_autoadj_sw(unsigned logn, uint32_t *restrict a,
	const uint32_t *restrict gm, uint32_t p, uint32_t p0i)
{
#if HAWK_AVX2
	__m256i yp = _mm256_set1_epi32(p);
...
/*
 * Transparent dispatcher for mp_NTT_autoadj(). Splits the function into
 * its two structurally different parts (see ntt_engine.sv's header
 * comment for the full derivation):
 *
 *   1. The O(n/4) "unfold" step (single twiddle gm[1], reflected
 *      a[u]/a[hn-u] indexing) -- always software, ntt_engine.sv does not
 *      implement this addressing pattern.
 *   2. The "reduced butterfly" phase (lm=1..logn-1) -- hardware-eligible:
 *      calling ntt_engine with job_logn=(logn-1) and MODE=2 (autoadj)
 *      reproduces it exactly, since the engine's per-stage butterfly
 *      count already determines how many `u` values get visited.
 *
 * AVX2 is untouched (mp_NTT_autoadj_sw() as a whole, unchanged) since
 * that targets x86, not this RISC-V/CVA6 SoC.
 */
static void
	...
	const uint32_t *restrict gm, uint32_t p, uint32_t p0i)
{
	...
	/* unfold step -- identical to mp_NTT_autoadj_sw()'s, always software */
	for (size_t u = 1; u < qn; u ++) {
		uint32_t x1 = a[u];
		uint32_t x2 = a[hn - u];
		a[u] = mp_sub(x1, mp_montymul(x2, s1, p, p0i), p);
	...
	if ((logn - 1) >= NTT_HW_MIN_LOGN) {
		mp_NTT_hw(logn - 1, a, gm, p, p0i, 2);
		vrfy_ntt_dispatch_cycles += read_csr(mcycle) - c0;
		vrfy_ntt_dispatch_calls  += 1;
		return;
	}

	/* fallback: reduced-butterfly phase, identical to
	   mp_NTT_autoadj_sw()'s (logn-1 < NTT_HW_MIN_LOGN never happens given
	   this function's own "Assumption: logn >= 3", kept for robustness) */
	{
		size_t t = hn;
		for (unsigned lm = 1; lm < logn; lm ++) {
			size_t m = (size_t)1 << lm;
	...
	vrfy_ntt_dispatch_calls  += 1;
#else
	mp_NTT_autoadj_sw(logn, a, gm, p, p0i);
#endif
}
...
#endif // HAWK_AVX2
	{
		uint64_t _c0 = read_csr(mcycle);
		fx32_FFT(logn, ft1);
		vrfy_fx32fft_dispatch_cycles += read_csr(mcycle) - _c0;
		vrfy_fx32fft_dispatch_calls  += 1;
	}
