# hawk512-opt — HAWK-512 with CVA6 hardware acceleration

This is `tests/hawk512/` (the plain, pure-software HAWK-512 KAT) with the
...
```
bash tests/hawk512-opt/run.sh
```

## Files changed vs. `tests/hawk512/`, and what replaced what
...
| `hawk_sign.c` | 2, 10–89 | `sig_gauss()`'s per-block squeeze offload to `gauss_sampler.sv` (`sig_gauss_hw_job()`) — **guarded by `logn==8`, so inert here**; HAWK-512's Gaussian sampling still runs entirely in software (see "Not accelerated" below) |
...
- **Gaussian sampling** (`sig_gauss()`, `hawk_sign.c`). `gauss_sampler.sv`'s
  ...
  comment: "for HAWK-256 (n=256) only"). HAWK-512 uses a different sigma
...
HAWK-512 (n≤512), and kept non-overlapping so KeyGen/Sign/Verify's
...
| `NTT_HW_SCRATCH_ADDR` (Verify) | `0x80F01000` | 2048B (n×4) | `hawk_vrfy.c` |
| `NTT_HW_SCRATCH_ADDR` (KeyGen/Sign) | `0x80F02000` | 2048B (n×4) | `ng_mp31.c` |
| `FFT_HW_SCRATCH_ADDR` | `0x80F03000` | 4096B (n×8) | `ng_fxp.c` |

## Provenance

`sha3.c`/`sha3.h`/`hawk_sign.c` (Keccak-AXI wiring) were already present
in this directory before this port (carried over from `hawk-512-keccak`,
...
direct copy plus scratch-window address/size adjustment for HAWK-512,
not a re-derivation.
