...
#if (PLATFORM_BYTE_ORDER == IS_LITTLE_ENDIAN)
    unsigned int i = 0;
    /* data (and occasionally state) may not be 8-byte aligned on targets
     * without hardware support for misaligned load/store (e.g. CVA6, which
     * traps). Always take the safe byte-wise path unless proven aligned,
     * rather than relying on a NO_MISALIGNED_ACCESSES build flag. */
...
void KeccakP1600_ExtractAndAddLanes(const void *state, const unsigned char *input, unsigned char *output, unsigned int laneCount)
{
    ...
    for(i=0; i<laneCount; i++) {
#if (PLATFORM_BYTE_ORDER == IS_LITTLE_ENDIAN)
        /* input/output are caller-supplied byte buffers and may not be
         * 8-byte aligned; state is always the internal aligned lane array. */
        uint64_t in_lane, out_lane;
        memcpy(&in_lane, input + i*8, 8);
        out_lane = in_lane ^ ((const uint64_t*)state)[i];
        memcpy(output + i*8, &out_lane, 8);
#else
...
size_t KeccakF1600_FastLoop_Absorb(void *state, unsigned int laneCount, const unsigned char *data, size_t dataByteLen)
{
    ...
    #endif
    uint64_t *stateAsLanes = (uint64_t*)state;
    /* data is a caller-supplied byte buffer and may not be 8-byte aligned;
     * copy each chunk into an aligned local buffer before feeding it to
     * addInput(), instead of casting data itself to uint64_t*. laneCount is
     * at most 21 (the largest Keccak-p[1600] rate in lanes). */
    uint64_t alignedInput[21];
    const unsigned char *curData = data;

    copyFromState(A, stateAsLanes)
    while(dataByteLen >= laneCount*8) {
        memcpy(alignedInput, curData, laneCount*8);
        addInput(A, alignedInput, laneCount)
        rounds24
        curData += laneCount*8;
...
size_t KeccakP1600_12rounds_FastLoop_Absorb(void *state, unsigned int laneCount, const unsigned char *data, size_t dataByteLen)
{
    ...
    #endif
    uint64_t *stateAsLanes = (uint64_t*)state;
    /* see KeccakF1600_FastLoop_Absorb above for why data isn't cast directly */
    uint64_t alignedInput[21];
    const unsigned char *curData = data;
