...
#define MONTG_KYBER(dest, a, b) \
    asm volatile ( \
        ".insn r 0x7b, 0x1, 0x0, %[rd], %[r1], %[r2] \n" \
        : [rd] "=&r" (dest) \
        : [r1] "r" (a), [r2] "r" (b) \
    );
...
int main(void) {
    ...
    0, 1, -1, 1, 1234567, -1234567, 2147483647, -2147483648, 0x0000FFFF,0xFFFF0000, 3329, 2*3329,  -3329,
    ...
    for (int i = 0; i < N_TESTS; i+=4) {
        //got_sw[i] = montgomery_reduce_kem(kem_inputs[i]);
        packed_a = ((uint64_t)(uint32_t)kem_inputs[i]) |
                   ((uint64_t)(uint32_t)kem_inputs[i+1] << 32);
        packed_b = ((uint64_t)(uint32_t)kem_inputs[i+2]) |
                   ((uint64_t)(uint32_t)kem_inputs[i+3] << 32);
        
        MONTG_KYBER(packed_result, packed_a, packed_b);
        
        got_sw[i]   = (int16_t)(packed_result & 0xFFFF);
        got_sw[i+1] = (int16_t)((packed_result >> 16) & 0xFFFF);
        got_sw[i+2] = (int16_t)((packed_result >> 32) & 0xFFFF);
        got_sw[i+3] = (int16_t)((packed_result >> 48) & 0xFFFF);
        
        //MONTG_KYBER(got_sw[i], (int64_t*) kem_inputs[i]);
    }
