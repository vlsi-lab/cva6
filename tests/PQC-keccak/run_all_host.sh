#!/bin/bash
#
# Batch-builds and runs every PQC-keccak variant's host Makefile (native gcc,
...
#
# Usage:
#   tests/PQC-keccak/run_all_host.sh [--variants v1,v2,...]
#
# Examples:
#   tests/PQC-keccak/run_all_host.sh                       # all 6 variants
#   tests/PQC-keccak/run_all_host.sh --variants ML-DSA-2,ml-kem-512
...
cd "$REPO_ROOT/tests/PQC-keccak" || exit 1

TS="$(date +%Y%m%d_%H%M%S)"
RESULTS_DIR="$REPO_ROOT/tests/PQC-keccak/bench_results_host/$TS"
mkdir -p "$RESULTS_DIR"
