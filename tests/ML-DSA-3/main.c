...
int main(void)
{
    ...
    //*************************************************
        ...
        print_uart_dec(cycles);
        print_uart("\n");
        if(memcmp(sm, TVEC_IN_SM_SIGN, 3342)) { printf("ERROR: SM mismatch\n");}
        //printVect("sm", sm, 3342);
    ...
    //*************************************************
        ...
        #endif /* TEST_KEY */
        #if TEST_SIGN == 0 
          memcpy(sm, TVEC_IN_SM_SIGN, 3342);
        #endif /* TEST_SIGN */
      
        write_csr(mcycle, 0);
       crypto_sign_open(m1, &mlen1, sm, 3342, NULL, 0, pk);
        cycles = read_csr(mcycle);
