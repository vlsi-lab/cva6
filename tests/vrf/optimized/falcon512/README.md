...
| `inner.h` | Added `hw_seen` field to `inner_shake256_context` (residency-tracking flag) |
...
| `falcon512_optimized.c` (formerly `main.c`) | Added `falcon_ntt_dispatch_cycles`/`falcon_ntt_dispatch_calls`, `falcon_hash_to_point_cycles`/`_calls`, `falcon_is_short_cycles`/`_calls` reporting after "verify" |
| `run.sh` | Points at this directory's own sources instead of `tests/falcon512/`'s, and adds `-I../../vrf_ip/sw` for `vrf_axi.h` |
...
`hw_owner == sc` stays true across the whole loop. This is the payoff of
keeping the state resident in the accelerator instead of re-absorbing/
re-uploading between related calls, applied here to Falcon's single
long-extract session.
...
Falcon's values), so a different modulus would work without RTL changes as
long as `thresh <= 5*q` keeps the 4-stage reduction exact.
