...
/*
 * ExpandA's per-entry matrix-A sampling (SHAKE128(rho||nonce), 3-byte
 * little-endian candidates masked to 23 bits, reject >= Q), HW-offloaded to
 * the shared vrf_ip rejection sampler (vrf_ip/rtl/rej_sampler.sv) in its
 * ML-DSA mode (REJ_CTRL.CAND3/RATE168/OUTWIDE). rej_sampler's own FSM
 * autonomously re-permutes and continues squeezing across as many
 * SHAKE128-rate blocks as needed to satisfy N=256 candidates -- the same
 * "one HW job replaces the whole squeeze-and-reject loop" shape as Falcon's
 * existing Zf(hash_to_point_hw)() (optimized/falcon512/shake.c). Validated
 * standalone (34-byte seed = 32-byte rho + 2-byte little-endian nonce,
 * matching dilithium_shake128_stream_init()'s own absorb exactly) in
 * tests/app-tests/rej-mldsa before being used here.
 */
                  ...
                  uint16_t nonce)
{
   ...
   * (pre-permutation, per rej_sampler.sv's precondition), same as
   * dilithium_shake128_stream_init()'s seed||nonce absorb + pad10*1. */
  for (i = 0; i < 200; i++) {
    block[i] = 0;
  }
  ...
  block[SEEDBYTES]     = (uint8_t)nonce;
  block[SEEDBYTES + 1] = (uint8_t)(nonce >> 8);
  block[SEEDBYTES + 2] ^= 0x1F;
  block[STREAM128_BLOCKBYTES - 1] ^= 0x80;
  for (i = 0; i < 25; i++) {
    uint64_t w = 0;
    for (unsigned b = 0; b < 8; b++) {
      w |= (uint64_t)block[8u * i + b] << (8u * b);
    }
    data[i] = w;
  }
