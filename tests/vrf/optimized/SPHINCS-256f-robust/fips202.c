...
/*
 * HW-offloaded: the software 24-round permutation (kept nowhere in this
 * file -- see the original PQClean-derived source for the pure-software
 * version) is replaced by a dispatch to the shared vrf_ip Keccak core via
 * the same CSREG/DATA[] raw-permute interface every other job on this
 * accelerator uses. All of keccak_absorb()/keccak_squeezeblocks()/
 * shake256_inc_absorb()/shake256_inc_squeeze() below call only this one
 * function for the permutation step, so accelerating it here covers every
 * SHAKE256 user in this file -- including hash_message() (verify's message
 * digest, not otherwise HW-offloaded) and thash_shake_{robust,simple}.c's
 * software fallback for inblocks>2 (FORS's root-aggregation hash, WOTS+'s
 * leaf hash) -- with no other call site needing changes.
 */
 ...
 * already uses. FRESH zeroes the resident state first; padding/domain
 * separation stays in software below, unchanged (the hardware's pad10*1
 * is hardcoded to SHAKE's 0x1F byte, matching every caller of this
 * function, but keeping the tail in software avoids needing a HW dispatch
 * at all for inputs under one rate block -- the common case for most
 * calls, and this only helps when mlen >= r anyway).
 */
static void
keccak_dma_absorb_fresh(const void *m, uint32_t len)
{
    ...
    *job_src_addr = (uint64_t)(uintptr_t)m;
    *job_src_len  = len;
...
static void keccak_absorb(uint64_t *s, uint32_t r, const uint8_t *m,
    ...
    if (full_len > 0) {
        keccak_dma_absorb_fresh(m, (uint32_t)full_len);
        for (i = 0; i < 25; ++i) {
            s[i] = cryptoState[i];
        }
        m += full_len;
