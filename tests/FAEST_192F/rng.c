...
/* Define NO_SEEDEXPANDER when linking with a library that already provides
 * its own seedexpander_init / seedexpander (avoids a duplicate-symbol
 * link error). Not currently exercised by any family in this repo -- kept
 * as an escape hatch for one that needs it. */
#ifndef NO_SEEDEXPANDER
/*
 seedexpander_init()
