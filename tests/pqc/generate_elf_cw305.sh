#!/usr/bin/env bash
# CW305-style build of PQC .elf files. Same CLI as generate_elf.sh, but with:
...
usage() {
  cat <<'EOF'
Usage:
  tests/pqc/generate_elf_cw305.sh [options] [FOLDER_NAME]

Generate RISC-V .elf files for PQC folders with CW305 build flags.
...
Options:
  ...
  -o, --out <path>                        Output root (default: tests/pqc/generated_elf_cw305)
...
FOLDER_NAME="all"
TEST_MODE="auto"
OUT_ROOT="$REPO_ROOT/tests/pqc/generated_elf_cw305"
CLEAN_ONLY=0
...
if [[ "$TEST_MODE" != "auto" && "$TEST_MODE" != "keygen" && "$TEST_MODE" != "sign" && "$TEST_MODE" != "sign-open" ]]; then
  echo "Invalid test mode: $TEST_MODE" >&2; exit 1
fi

if [[ $CLEAN_ONLY -eq 1 ]]; then
  if [[ ! -d "$OUT_ROOT" ]]; then
    echo "No output directory to clean: $OUT_ROOT"; exit 0
  fi
  elf_count="$(find "$OUT_ROOT" -type f -name '*.elf' | wc -l)"
  if [[ "$elf_count" -eq 0 ]]; then
    echo "No .elf files found under: $OUT_ROOT"; exit 0
  fi
  find "$OUT_ROOT" -type f -name '*.elf' -delete
  echo "Removed $elf_count .elf file(s) under: $OUT_ROOT"; exit 0
fi
...
if [[ ! -f ./verif/sim/setup-env.sh ]]; then
  echo "Error: missing ./verif/sim/setup-env.sh" >&2; exit 1
fi
...
if [[ -z "${RISCV_CC:-}" ]]; then
  echo "Error: RISCV_CC is not set after setup-env.sh" >&2; exit 1
fi

# CW305 ISA/ABI defaults (overridable via env)
...
compile_one() {
  ...
  local test_name src_base thash_file elf elf_name

  test_name="$(basename "$rel_from_pqc")"

  if [[ ! -f "$test_dir/main.c" ]]; then
    echo "[skip] missing main.c: $rel_from_pqc"; return
  fi
  ...
  if [[ -z "$thash_file" ]]; then
    echo "[skip] missing thash source in: $rel_from_pqc"; return
  fi
  ...
  # CW305 common runtime (cw305_crt.S replaces crt.S)
  local -a src_common=(
    ../tests/custom/common/syscalls.c
    ../tests/custom/common/cw305_crt.S
  )

  # CW305-style compilation flags
  local -a cflags=(
...
if [[ $fail_count -gt 0 ]]; then
  echo
  echo "Completed with $fail_count failure(s) and $built_count success(es)." >&2; exit 1
fi
