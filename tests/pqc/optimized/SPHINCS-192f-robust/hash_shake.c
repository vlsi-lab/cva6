...
/* Execute OP_PRF_192: performs PRF using pub_seed, addr, and sk_seed (FIPS 205, n=24)
 * Input: reg[0:5]=pub_seed (24 bytes), reg[6:13]=addr (32 bytes), reg[14:19]=sk_seed (24 bytes)
 * Output: reg[0:5]=PRF result (24 bytes)
 * Computes: SHAKE256(pub_seed || addr || sk_seed)[0:SPX_N]
 */
extern void prf_192_hw_compute(void);
              ...
              const uint32_t addr[8])
{
    ...
    /* Load pub_seed to reg[0:5] (24 bytes) - 1 fused LOAD2 + 1 cus_load. */
    cus_load2_pair(load32_prf(ctx->pub_seed + 0), load32_prf(ctx->pub_seed + 4),
                   load32_prf(ctx->pub_seed + 8), load32_prf(ctx->pub_seed + 12), 0);
    cus_load(load32_prf(ctx->pub_seed + 16), load32_prf(ctx->pub_seed + 20), 4);

    /* Load addr to reg[6:13] (32 bytes) - 2 fused LOAD2. */
    for (i = 0; i < 8; i += 4) {
        cus_load2_pair(load32_prf(addr_bytes + 4*i),     load32_prf(addr_bytes + 4*(i+1)),
                       load32_prf(addr_bytes + 4*(i+2)), load32_prf(addr_bytes + 4*(i+3)), 6 + i);
    }

    /* Load sk_seed to reg[14:19] (24 bytes) - 1 fused LOAD2 + 1 cus_load. */
    cus_load2_pair(load32_prf(ctx->sk_seed + 0), load32_prf(ctx->sk_seed + 4),
                   load32_prf(ctx->sk_seed + 8), load32_prf(ctx->sk_seed + 12), 14);
    cus_load(load32_prf(ctx->sk_seed + 16), load32_prf(ctx->sk_seed + 20), 18);
