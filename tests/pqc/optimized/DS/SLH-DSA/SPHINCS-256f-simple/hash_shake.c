...
/* Execute OP_PRF_256: performs PRF using pub_seed, addr, and sk_seed (FIPS 205, n=32)
 * Input: reg[0:7]=pub_seed (32 bytes), reg[8:15]=addr (32 bytes), reg[16:23]=sk_seed (32 bytes)
 * Output: reg[0:7]=PRF result (32 bytes)
 * Computes: SHAKE256(pub_seed || addr || sk_seed)[0:SPX_N]
 */
extern void prf_256_hw_compute(void);
              ...
              const uint32_t addr[8])
{
    ...
    /* Load pub_seed to reg[0:7] (32 bytes) - 2 fused LOAD2. */
    cus_load2_pair(load32_prf(ctx->pub_seed + 0), load32_prf(ctx->pub_seed + 4),
                   load32_prf(ctx->pub_seed + 8), load32_prf(ctx->pub_seed + 12), 0);
    cus_load2_pair(load32_prf(ctx->pub_seed + 16), load32_prf(ctx->pub_seed + 20),
                   load32_prf(ctx->pub_seed + 24), load32_prf(ctx->pub_seed + 28), 4);

    /* Load addr to reg[8:15] (32 bytes) - 2 fused LOAD2. */
    for (i = 0; i < 8; i += 4) {
        cus_load2_pair(load32_prf(addr_bytes + 4*i),     load32_prf(addr_bytes + 4*(i+1)),
                       load32_prf(addr_bytes + 4*(i+2)), load32_prf(addr_bytes + 4*(i+3)), 8 + i);
    }

    /* Load sk_seed to reg[16:23] (32 bytes) - 2 fused LOAD2. */
    cus_load2_pair(load32_prf(ctx->sk_seed + 0), load32_prf(ctx->sk_seed + 4),
                   load32_prf(ctx->sk_seed + 8), load32_prf(ctx->sk_seed + 12), 16);
    cus_load2_pair(load32_prf(ctx->sk_seed + 16), load32_prf(ctx->sk_seed + 20),
                   load32_prf(ctx->sk_seed + 24), load32_prf(ctx->sk_seed + 28), 20);
