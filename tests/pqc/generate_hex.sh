...
usage() {
  cat <<'EOF'
Usage:
  tests/pqc/generate_hex.sh [options] [FOLDER_NAME]

Generate FPGA-ready .hex files for PQC folders without running ISS/simulation.
...
Options:
  ...
  -o, --out <path>                        Output root (default: tests/pqc/generated_hex)
      --clean                              Remove all .hex files under output root and exit
  -h, --help                              Show this help

Examples:
  tests/pqc/generate_hex.sh
  tests/pqc/generate_hex.sh --variant optimized
  tests/pqc/generate_hex.sh --name SPHINCS-128f-robust
  tests/pqc/generate_hex.sh SPHINCS-256f-simple
  tests/pqc/generate_hex.sh --test keygen
  tests/pqc/generate_hex.sh --variant baseline --test-sign-open
  tests/pqc/generate_hex.sh --clean
EOF
}
...
FOLDER_NAME="all"
TEST_MODE="auto"
OUT_ROOT="$REPO_ROOT/tests/pqc/generated_hex"
CLEAN_ONLY=0
...
if [[ $CLEAN_ONLY -eq 1 ]]; then
  ...
  hex_count="$(find "$OUT_ROOT" -type f -name '*.hex' | wc -l)"
  if [[ "$hex_count" -eq 0 ]]; then
    echo "No .hex files found under: $OUT_ROOT"
    exit 0
  fi

  find "$OUT_ROOT" -type f -name '*.hex' -delete
  echo "Removed $hex_count .hex file(s) under: $OUT_ROOT"
  exit 0
fi
...
if [[ -z "${RISCV_CC:-}" || -z "${RISCV_OBJCOPY:-}" ]]; then
  echo "Error: RISCV_CC/RISCV_OBJCOPY are not set after setup-env.sh" >&2
  exit 1
fi
...
compile_one() {
  ...
  if [[ "$TEST_MODE" = "auto" ]]; then
    hex_name="${test_name}.hex"
  else
    hex_name="${test_name}-test-${TEST_MODE}.hex"
  fi
  hex="$out_test_dir/${hex_name}"
  ...
  if ! "$RISCV_OBJCOPY" -O verilog "$elf" "$hex"; then
    fail_count=$((fail_count + 1))
    echo "[fail] objcopy failed: $rel_from_pqc" >&2
    return
  fi

  built_count=$((built_count + 1))
  echo "[ok] $rel_from_pqc -> $hex"
}
...
echo
echo "Generated $built_count hex file(s) under: $OUT_ROOT"
