#!/usr/bin/env bash
# CW305-style build of PQC .hex files. Same CLI as generate_hex.sh, but with:
...
usage() {
  cat <<'EOF'
Usage:
  tests/pqc/generate_hex_cw305.sh [options] [FOLDER_NAME]

Generate FPGA-ready .hex files for PQC folders with CW305 build flags.
...
Options:
  ...
  -o, --out <path>                        Output root (default: tests/pqc/generated_hex_cw305)
...
OUT_ROOT="$REPO_ROOT/tests/pqc/generated_hex_cw305"
...
if [[ $CLEAN_ONLY -eq 1 ]]; then
  ...
  if [[ "$hex_count" -eq 0 ]]; then echo "No .hex files found under: $OUT_ROOT"; exit 0; fi
  find "$OUT_ROOT" -type f -name '*.hex' -delete
  echo "Removed $hex_count .hex file(s) under: $OUT_ROOT"; exit 0
fi
...
BUILD_DIR="$(mktemp -d "$REPO_ROOT/verif/sim/.pqc_hex_cw305_build.XXXXXX")"
...
compile_one() {
  ...
  local test_name src_base thash_file elf hex hex_name
