...
usage() {
  cat <<'EOF'
Usage:
  tests/pqc/generate_bin.sh [options] [FOLDER_NAME]

Generate raw .bin files for PQC folders without running ISS/simulation.
...
Options:
  ...
  -o, --out <path>                        Output root (default: tests/pqc/generated_bin)
      --clean                              Remove all .bin files under output root and exit
  -h, --help                              Show this help

Examples:
  tests/pqc/generate_bin.sh
  tests/pqc/generate_bin.sh --variant optimized
  tests/pqc/generate_bin.sh --name SPHINCS-128f-robust
...
FOLDER_NAME="all"
TEST_MODE="auto"
OUT_ROOT="$REPO_ROOT/tests/pqc/generated_bin"
CLEAN_ONLY=0
...
if [[ $CLEAN_ONLY -eq 1 ]]; then
  ...
  if [[ "$bin_count" -eq 0 ]]; then
    echo "No .bin files found under: $OUT_ROOT"
    exit 0
  fi

  find "$OUT_ROOT" -type f -name '*.bin' -delete
  echo "Removed $bin_count .bin file(s) under: $OUT_ROOT"
  exit 0
fi
...
BUILD_DIR="$(mktemp -d "$REPO_ROOT/verif/sim/.pqc_bin_build.XXXXXX")"
