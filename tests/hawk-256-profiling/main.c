...
// Date: June 2025                                                                 //
// Desc: Entry point for cycle-accurate profiling of HAWK-256 on CVA6.            //
...
#include "profiling.h"  /* shared print_cycles + CSR helpers                    */

/*
 * Runtime profiling flag — controls which sub-function level is printed.
 *   0 = silent (no PROF_END output)
 *   1 = L1 sub-functions (keygen: Hawk_keygen/encode_public/encode_private;
 *       sign: Hawk_regen_fg/sign_hm/sign_h/…; verify: decode_public_key/…)
 *   2 = L1 + L2 sub-sub-functions (encode_gr_q00, vrfy_shake, …)
 * Change this value before each function call to isolate the level you want.
 */
int prof_print_level = 2;

/* Default: all phases enabled. Override per phase with -DTEST_KEY=0, etc. */
#ifndef TEST_KEY
#define TEST_KEY       1
#endif
#ifndef TEST_SIGN
...
/* print_cycles is provided by profiling.h */
