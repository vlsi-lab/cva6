	...
	const void *pub, size_t pub_len)
{
	...
	PROF_END2("extract_lowbit_F");
	buf += (n >> 3);
	PROF_BEGIN2();
	extract_lowbit(logn, buf, G);
	PROF_END2("extract_lowbit_G");
	buf += (n >> 3);
	...
	PROF_END2("encode_private_shake");
	buf += hpub_len;
	...
	void *restrict tmp, size_t tmp_len)
{
	...
	for (;;) {
		PROF_BEGIN1();
		int kgen_err = Hawk_keygen(logn, f, g, 0, 0, 0, 0, 0, 0,
			rng, rng_context,
			tt8, (size_t)(((int8_t *)tmp + tmp_len) - tt8));
		PROF_END1("Hawk_keygen");
		if (kgen_err != 0) {
			return 0;
		}

		PROF_BEGIN1();
		int pub_ok = encode_public(logn, tpub, pub_len, q00, q01);
		PROF_END1("encode_public");
		if (pub_ok) {
			PROF_BEGIN1();
			(void)encode_private(logn, tpriv,
				seed, F, G, tpub, pub_len);
			PROF_END1("encode_private");
#if HAWK_DEBUG
			printf("#### Keygen (n=%u):\n", 1u << logn);
