	...
	const void *priv, size_t priv_len, void *tmp, size_t tmp_len)
{
	...
	for (uint32_t attempt = 0;; attempt += 2) {
		...
		PROF_BEGIN1();
		int _enc_ok = encode_sig(logn, tmp, sig_len, salt, salt_len, s1);
		PROF_END1("sign_encode_sig");
		if (_enc_ok) {
#if HAWK_DEBUG
