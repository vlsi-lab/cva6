...
#define GAUSS_HW_AXI_BASE 0x50000000UL

/*
 * The gauss_sampler hardware writes its output samples directly to CPU
 * memory as a second, independent AXI master -- bypassing the CPU's D$
 * entirely. This SoC's D$ is write-through (stores are never "dirty", so
 * nothing needs writing back), but it still caches *reads*: without care,
 * a later CPU read of a sample the hardware just wrote could still hit a
 * stale line left over from an earlier, unrelated cached access to the
 * same address (e.g. this buffer's own previous use), silently ignoring
 * the hardware's write.
 *
 * Rather than flip on a global "flush the whole D$ on every fence"
 * config bit (which would tax every *other*, unrelated fence in the
 * program -- including the ones shake_inject() already issues on every
 * absorb -- for a coherency problem that only this one code path has),
 * the sampler writes into this fixed scratch window instead of the
 * caller's real x[] buffer. GAUSS_HW_SCRATCH_ADDR sits just above
 * CachedRegionLength in cv64a6_imac_crypto_config_pkg.sv, so it is
 * uncached by construction: every access -- from the CPU or the
 * peripheral -- goes straight to memory, no coherency gap possible.
 * sig_gauss() copies the final result out of it with one ordinary
 * memcpy() once all 4 lanes are done.
 */
#define GAUSS_HW_SCRATCH_ADDR 0x80F00000UL
#define GAUSS_HW_SCRATCH_SIZE 512u /* 2*n, n<=256 (HAWK-256 only) */

/*
 * Offload sig_gauss()'s inner per-block "squeeze 40 bytes -> compute 4
 * samples" loop to the gauss_sampler hardware CDT sampler (HAWK-256 only
 * -- see keccak_ip/rtl/gauss_sampler.sv). The caller must have already
 * made sc the accelerator's hardware-resident context and applied
 * shake_flip()'s padding without permuting (i.e. call this immediately
 * after shake_flip(sc), with nothing else having touched the accelerator
 * since), matching every other resident-squeeze call site in sha3.c.
 * Writes into GAUSS_HW_SCRATCH_ADDR + bitbase (not the real x[] -- see
 * GAUSS_HW_SCRATCH_ADDR above); the caller is responsible for copying it
 * out once all 4 lanes are done. Returns the squared-norm contribution
 * for this job's nblocks blocks (4*nblocks samples), matching
 * sig_gauss()'s own per-j running sum.
 */
static uint32_t
sig_gauss_hw_job(unsigned bitbase, const uint8_t *t, unsigned nblocks)
{
	uint64_t volatile *samp_t_addr =
	    (uint64_t volatile *)(GAUSS_HW_AXI_BASE + KECCAK_SAMP_T_ADDR_REG_OFFSET);
	uint64_t volatile *samp_x_addr =
	    (uint64_t volatile *)(GAUSS_HW_AXI_BASE + KECCAK_SAMP_X_ADDR_REG_OFFSET);
	uint64_t volatile *samp_bitbase =
	    (uint64_t volatile *)(GAUSS_HW_AXI_BASE + KECCAK_SAMP_BITBASE_REG_OFFSET);
	uint64_t volatile *samp_nblocks =
	    (uint64_t volatile *)(GAUSS_HW_AXI_BASE + KECCAK_SAMP_NBLOCKS_REG_OFFSET);
	uint64_t volatile *samp_ctrl =
	    (uint64_t volatile *)(GAUSS_HW_AXI_BASE + KECCAK_SAMP_CTRL_REG_OFFSET);
	uint64_t volatile *samp_sn =
	    (uint64_t volatile *)(GAUSS_HW_AXI_BASE + KECCAK_SAMP_SN_REG_OFFSET);
	uint32_t sn;

	/*
	 * Ensure the CPU's stores into *t (and, on a fresh/uploaded context
	 * switch, into the DATA[] registers) have reached memory before the
	 * accelerator's AXI master reads them -- ordinary MMIO/DMA
	 * ordering, unrelated to the cache-coherency concern above.
	 */
	__asm__ volatile ("fence" ::: "memory");

	*samp_t_addr  = (uint64_t)(uintptr_t)t;
	*samp_x_addr  = (uint64_t)GAUSS_HW_SCRATCH_ADDR + bitbase;
	...
	const shake_context *sc_extra, int8_t *x, const uint8_t *t)
{
	...
	for (size_t j = 0; j < 4; j ++) {
		...
		if (logn == 8) {
			sn += sig_gauss_hw_job((unsigned)(j << 2), t,
				(unsigned)(n >> 3));
			continue;
		}
	...
	if (logn == 8) {
		memcpy(x, (const void *)GAUSS_HW_SCRATCH_ADDR,
			GAUSS_HW_SCRATCH_SIZE);
	}
	return sn;
