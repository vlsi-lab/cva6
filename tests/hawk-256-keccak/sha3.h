...
/*
 * Clone a SHAKE context: dst becomes an independent copy of src's current
 * state. Equivalent to "*dst = *src" but also safe when src is the
 * accelerator's currently hardware-resident context (a plain struct copy
 * would silently miss bytes absorbed since src last became resident, since
 * those only exist in the accelerator's registers until evicted).
 */
void shake_clone(shake_context *dst, const shake_context *src);
