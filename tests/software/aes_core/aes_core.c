// AES-128 key expansion core benchmark, pure software -- the AES analogue of
// keccak_core's single permutation call (the algorithmic setup/schedule
// primitive, kept distinct from the aes_encrypt/aes_decrypt/aes_cbc
// application benchmarks). Expected round keys are the standard FIPS-197
// Appendix A.1 example (key 000102030405060708090a0b0c0d0e0f).

#include "uart.h"
#include "encoding.h"
#include "bench.h"
#include "aes128_block.h"

static const uint8_t KEY[16] = {
    0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
    0x0c, 0x0d, 0x0e, 0x0f,
};

static const uint8_t EXPECTED[176] = {
    ,
};

int main()
{
  aes128_ctx_t ctx;
  int errors = 0;
  unsigned long cycles, instrs;

  printf("AES-128 Key Expansion Core Benchmark - Software\n");

  BENCH_ENABLE();
  BENCH_START();
  aes128_key_expand(&ctx, KEY);
  BENCH_READ(cycles, instrs);

  printf("cycles=%lu instrs=%lu\n", cycles, instrs);

  for (int i = 0; i < 176; i++) {
    if (ctx.RoundKey[i] != EXPECTED[i]) {
      printf("!!! Mismatch at byte %d: expected 0x%02x, got 0x%02x !!!\n", i, EXPECTED[i], ctx.RoundKey[i]);
      errors++;
    }
  }

  if (errors == 0) printf("AES-128 Key Expansion Core Benchmark terminated with no errors.\n");
  else              printf("AES-128 Key Expansion Core Benchmark terminated with %d errors\n", errors);

  return errors;
}
