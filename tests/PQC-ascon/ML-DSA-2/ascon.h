 ...
 *
 * All of Dilithium's symmetric primitives (matrix/mask expansion, challenge
 * hashing, message hashing) are served by the Ascon permutation below.
 */
...
/* 12-round permutation, offloaded to the Ascon AXI accelerator */
static inline void ascon_p12(ascon_permutation_state_t* s) {
  ...
  *csreg &= ~(1ULL << ASCON_CSREG_START_BIT);
  ...
  /* Copy crypto state back from Ascon AXI accelerator */
  s->x0 = cryptoState[0];
  s->x1 = cryptoState[1];
  s->x2 = cryptoState[2];
  s->x3 = cryptoState[3];
