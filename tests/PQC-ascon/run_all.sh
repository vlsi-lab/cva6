#!/bin/bash
#
# Batch-runs every PQC-ascon variant's run.sh (software-only and/or AXI
...
#
# Usage:
#   tests/PQC-ascon/run_all.sh [--only sw|hw|both] [--variants v1,v2,...] [--timeout SECONDS]
#
# Examples:
#   tests/PQC-ascon/run_all.sh                       # all 6 variants, sw + hw
...
RESULTS_DIR="$REPO_ROOT/tests/PQC-ascon/bench_results/$TS"
mkdir -p "$RESULTS_DIR"
...
for variant in "${VARIANTS[@]}"; do
  run_sh="tests/PQC-ascon/$variant/run.sh"
  if [ ! -f "$run_sh" ]; then
