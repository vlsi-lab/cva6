...
// clmul_native.h). Same ghash()/gf128_mul() interface and byte/bit
// semantics as tests/software/common/ghash.c (bit-serial reference) and
// tests/tightly/common/ghash.c (coprocessor variant), but gf128_mul here
// is computed via bit-reflection + a schoolbook 64x64->128 carry-less
// multiply (4 partial products from clmul/clmulh) + shift-xor reduction,
// the standard technique for computing GHASH with a native carry-less
// multiplier (see e.g. Gueron & Kounavis, "Carry-Less Multiplication and
// Its Usage for Computing the GCM Mode", Intel whitepaper 2010).
//
...
// random 16-byte pairs in a standalone Python/host-C harness before being
// ported here -- see the task notes for the validation script and output.
//
