...
/*
 * Cycle accounting for the vect_FFT()/vect_iFFT() dispatchers below, same
 * pattern as ng_mp31.c's ntt_dispatch_cycles/ntt_dispatch_calls -- added
 * to measure babai_loop's (solve_NTRU_intermediate, ng_ntru.c) actual
 * FFT-call share of KeyGen, versus poly_sub_scaled and the rest of
 * ng_zint31.c's big-integer arithmetic. See NTT_ACCEL_DESIGN.md's
 * "Fixed-point FFT scoping" section. Every caller (ng_hawk.c's
 * constant-term check, ng_ntru.c's babai_loop and solve_NTRU_depth0)
 * already funnels through these two functions.
 */
uint64_t fft_dispatch_cycles = 0;
uint64_t fft_dispatch_calls  = 0;
...
/*
 * Hardware/software dispatcher for vect_FFT()/vect_iFFT() (ntt_engine.sv
 * Revision 3, see its header comment and NTT_ACCEL_DESIGN.md's "Fixed-
 * point FFT scoping" section). `f` is an ordinary cached CPU buffer, and
 * the peripheral writes results back as a second AXI master, so it is
 * staged through a fixed non-cacheable DRAM scratch window -- same
 * convention as GAUSS_HW_SCRATCH_ADDR (hawk_sign.c) and the various
 * NTT_HW_SCRATCH_ADDR windows (ng_mp31.c/hawk_vrfy.c), at a fourth,
 * distinct address so none of them ever alias. GM_TAB needs no such
 * treatment: it's a fixed ROM inside the engine (fft_gm_rom.sv), not
 * fetched from DRAM at all.
 *
 * job_logn_i sent to hardware is (real logn - 1) -- the engine's own
 * address-generator reuse trick, see ntt_engine.sv's header comment.
 * The size threshold below (FFT_HW_MIN_LOGN) mirrors NTT_HW_MIN_LOGN's
 * reasoning: below it, fixed per-job overhead exceeds the savings.
 */
#define KECCAK_AXI_BASE_ADDR 0x50000000UL
#define FFT_HW_SCRATCH_ADDR  0x80F01000UL
#define FFT_HW_SCRATCH_SIZE  2048u   /* n*8, n<=256 (HAWK-256 only) */
#define FFT_HW_MIN_LOGN      2u

/* job_mode_i encoding (see ntt_engine.sv header comment): bit2=FFT family,
 * bit0=direction. */
#define FFT_MODE_FFT   4u
#define FFT_MODE_IFFT  5u

static void
vect_fft_hw(unsigned logn, fxr *restrict f, unsigned mode)
{
	size_t n = (size_t)1 << logn;
	fxr volatile *scratch = (fxr volatile *)FFT_HW_SCRATCH_ADDR;
	...
	*ntt_a_addr = (uint64_t)FFT_HW_SCRATCH_ADDR;
	*ntt_logn   = logn - 1;
 ...
 * fft_engine_test.c, the standalone hardware-vs-software correctness/
 * speedup test. */
TARGET_AVX2
void
vect_FFT_sw(unsigned logn, fxr *f)
{
...
TARGET_AVX2
void
vect_iFFT_sw(unsigned logn, fxr *f)
{
	size_t hn = (size_t)1 << (logn - 1);
	size_t ht = 1;
#if NTRUGEN_AVX2
...
/*
 * Transparent dispatchers: every existing caller (ng_hawk.c's constant-
 * term check, ng_ntru.c's babai_loop and solve_NTRU_depth0) keeps calling
 * vect_FFT()/vect_iFFT() unchanged -- same pattern as ng_mp31.c's
 * mp_NTT()/mp_iNTT() dispatchers.
 */
void
vect_FFT(unsigned logn, fxr *f)
{
	uint64_t c0 = read_csr(mcycle);
	if (logn >= FFT_HW_MIN_LOGN) {
		vect_fft_hw(logn, f, FFT_MODE_FFT);
	} else {
		vect_FFT_sw(logn, f);
	}
	fft_dispatch_cycles += read_csr(mcycle) - c0;
	fft_dispatch_calls  += 1;
}
...
vect_iFFT(unsigned logn, fxr *f)
{
	uint64_t c0 = read_csr(mcycle);
	if (logn >= FFT_HW_MIN_LOGN) {
		vect_fft_hw(logn, f, FFT_MODE_IFFT);
	} else {
		vect_iFFT_sw(logn, f);
	}
	fft_dispatch_cycles += read_csr(mcycle) - c0;
