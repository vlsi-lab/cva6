...
/*
 * Cycle accounting for the mp_NTT()/mp_iNTT() dispatchers below, added to
 * answer "how much of solve_NTRU()/KeyGen is actually NTT-call time,
 * versus other (ng_zint31.c CRT/big-integer) arithmetic" -- see
 * NTT_ACCEL_DESIGN.md. Every caller (ng_hawk.c, ng_ntru.c, ng_poly.c)
 * already funnels through these two functions, so timing them here
 * covers all 100+ call sites without touching any of them. Printed by
 * main.c right after "Keygen OK".
 */
...
/*
 * Hardware offload of mp_NTT()/mp_iNTT() to ntt_engine.sv
 * (keccak_ip/rtl/ntt_engine.sv). The engine takes p/p0i as plain per-job
 * register values (NTT_P_VAL/NTT_P0I_VAL) rather than hardcoding a fixed
 * prime pair, so -- unlike the first cut of this redirect -- ANY modulus
 * mp_NTT()/mp_iNTT() is ever called with works on hardware, not just P1/P2.
 * This lives on the real, exported Zn(mp_NTT)/Zn(mp_iNTT) symbols every
 * ntrugen-namespace caller uses (ng_hawk.c, ng_ntru.c, ng_poly.c) -- unlike
 * the earlier hawk_vrfy.c redirect, which only patches that file's own
 * private, non-exported mp_NTT copy -- so ng_hawk.c's KeyGen invertibility
 * pre-filter, make_q001, and ng_ntru.c's/ng_poly.c's full PRIMES[0..slen]
 * solve_NTRU() loops (KeyGen's actual 80% bottleneck) all get accelerated
 * now, not just the P1/P2 subset.
 *
 * The only remaining guard is a size threshold (logn>=2, i.e. n>=4): for
 * very small transforms the fixed per-job overhead (5 register writes,
 * the DONE poll, the scratch-window copy) is not worth it -- logn==0 is
 * already a no-op in both mp_NTT()/mp_iNTT() themselves, and logn==1 is a
 * single butterfly, cheaper done directly than dispatched.
 *
 * `a` is an ordinary cached CPU buffer (the callers' own tmp/scratch
 * arrays), and the peripheral writes results back to DRAM as a second AXI
 * master, so results are staged through a fixed non-cacheable DRAM
 * scratch window -- same convention as GAUSS_HW_SCRATCH_ADDR (hawk_sign.c)
 * and NTT_HW_SCRATCH_ADDR (hawk_vrfy.c), at a third, distinct address so
 * none of the three ever alias, even though KeyGen/Sign/Verify never run
 * concurrently in this KAT flow. `gm`/`igm` need no such treatment: for
 * every current caller they are either compile-time-constant tables or
 * populated once via mp_mkgm()/mp_mkgmigm() well before the transform, so
 * their DRAM contents are always correct regardless of CPU cache state.
 */
#define KECCAK_AXI_BASE_ADDR 0x50000000UL
#define NTT_HW_SCRATCH_ADDR  0x80F00600UL
#define NTT_HW_SCRATCH_SIZE  1024u   /* n*4, n<=256 (HAWK-256 only) */
#define NTT_HW_MIN_LOGN      2u      /* below this, dispatch overhead > savings */

/* External linkage (not static): also called directly, bypassing the
 * mp_NTT()/mp_iNTT() dispatchers below, by the standalone single-operation
 * benchmarks (ntt_hw_bench.c, ntt_hw_cost_breakdown.c) that isolate the
 * hardware path's own cost from the software path's. */
void
...
/* see ng_inner.h. External linkage (not static): also called directly by
 * ntt_sw_bench.c, the pure-software single-operation benchmark. */
TARGET_AVX2
void
mp_NTT_sw(unsigned logn, uint32_t *restrict a, const uint32_t *restrict gm,
...
/*
 * Transparent dispatcher: every existing caller (ng_hawk.c, ng_ntru.c,
 * ng_poly.c) keeps calling mp_NTT() unchanged. Hardware now handles any
 * modulus (see mp_NTT_hw()'s header comment); the only guard left is the
 * NTT_HW_MIN_LOGN size threshold.
 */
void
	...
	uint32_t p, uint32_t p0i)
{
	...
	ntt_dispatch_cycles += read_csr(mcycle) - c0;
	ntt_dispatch_calls  += 1;
}
...
TARGET_AVX2
void
mp_iNTT_sw(unsigned logn, uint32_t *restrict a, const uint32_t *restrict igm,
	uint32_t p, uint32_t p0i)
{
	if (logn == 0) {
		return;
...
/*
 * Transparent dispatcher, same rationale as mp_NTT() above -- now every
 * mp_iNTT() call in solve_NTRU()'s wide PRIMES[0..slen] loops is eligible
 * for hardware, not just ng_hawk.c's P1-only calls.
 */
void
	...
	uint32_t p, uint32_t p0i)
{
	uint64_t c0 = read_csr(mcycle);
	if (logn >= NTT_HW_MIN_LOGN) {
		mp_NTT_hw(logn, a, igm, p, p0i, 1);
	} else {
		mp_iNTT_sw(logn, a, igm, p, p0i);
	}
	ntt_dispatch_cycles += read_csr(mcycle) - c0;
	ntt_dispatch_calls  += 1;
}
