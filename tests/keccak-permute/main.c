...
/* ---------------- HW permute via the new HASH coprocessor ---------------- */
static void keccak_hw_full_permute(const uint64_t in[25], uint64_t out[25]) {
...
int main(void) {
    uint64_t sw_state[25];
    uint64_t hw_state[25];
    ...
    memcpy(hw_state, in_state, sizeof(in_state)); /* unused; HW writes to it */
