/*
 * Ascon XOF/Hash implementation for Dilithium
...
/*
 * ============================================================
 * SHAKE-compatible API (using Ascon-XOF underneath)
 * ============================================================
 */

/* SHAKE128-like functions */
...
void shake128_squeezeblocks(uint8_t *out, size_t nblocks, ascon_state *state) {
  ascon_xof_squeezeblocks(out, nblocks, state);
}
...
void sha3_256(uint8_t h[32], const uint8_t *in, size_t inlen) {
  ascon_hash_256(h, in, inlen);
}

void sha3_512(uint8_t h[64], const uint8_t *in, size_t inlen) {
  ascon_hash_512(h, in, inlen);
}
