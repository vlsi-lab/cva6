...
| `inner.h` | Added `hw_seen` field to `inner_shake256_context` (residency-tracking flag, same purpose as HAWK's `shake_context.hw_seen`); added `Zf(hash_to_point_hw)()` prototype |
...
| `run.sh` | Points at this directory's own sources instead of `tests/falcon1024/`'s, and adds `-I../../keccak_ip/sw` for `keccak_axi.h` |
...
is the same "store the state in the accelerator, don't re-absorb/re-upload
between related calls" optimization the write-up above describes for
HAWK's 3-session chain, applied to Falcon's single long-extract session
instead.
