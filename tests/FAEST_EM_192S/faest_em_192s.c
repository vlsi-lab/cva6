...
int FAEST_CALLING_CONVENTION faest_em_192s_unpack_private_key(faest_em_192s_unpacked_private_key_t* unpacked_sk, const uint8_t* sk) {
  if (!unpacked_sk || !sk) {
    return -1;
  }
  ...
  faest_em_192s_owf(SK_KEY(sk), SK_INPUT(sk), unpacked_sk->owf_output);
...
int FAEST_CALLING_CONVENTION faest_em_192s_sign_with_randomness(const uint8_t* sk, const uint8_t* message, size_t message_len, const uint8_t* rho, size_t rho_len, uint8_t* signature, size_t* signature_len) {
  if (!sk || !signature || !signature_len || *signature_len < FAEST_EM_192S_SIGNATURE_SIZE || (!rho && rho_len) || (!message && message_len)) {
    return -1;
  }

  faest_em_192s_unpacked_private_key_t unpacked_sk;
  int ret = faest_em_192s_unpack_private_key(&unpacked_sk, sk);
  if (ret) {
    faest_em_192s_clear_unpacked_private_key(&unpacked_sk);
    return ret;
  }

  ret = faest_em_192s_unpacked_sign_with_randomness(&unpacked_sk, message, message_len, rho, rho_len, signature, signature_len);
  faest_em_192s_clear_unpacked_private_key(&unpacked_sk);
  return ret;
}
...
int FAEST_CALLING_CONVENTION faest_em_192s_sign(const uint8_t* sk, const uint8_t* message, size_t message_len, uint8_t* signature, size_t* signature_len) {
  if (!sk || !signature || !signature_len || *signature_len < FAEST_EM_192S_SIGNATURE_SIZE || (!message && message_len)) {
    return -1;
  }
...
int FAEST_CALLING_CONVENTION faest_em_192s_unpacked_sign(const faest_em_192s_unpacked_private_key_t* sk, const uint8_t* message, size_t message_len, uint8_t* signature, size_t* signature_len) {
  if (!sk || !signature || !signature_len || *signature_len < FAEST_EM_192S_SIGNATURE_SIZE || (!message && message_len)) {
    return -1;
  }
  ...
  return faest_em_192s_unpacked_sign_with_randomness(sk, message, message_len, rho, sizeof(rho), signature, signature_len);
}

int FAEST_CALLING_CONVENTION faest_em_192s_unpacked_sign_with_randomness(const faest_em_192s_unpacked_private_key_t* sk, const uint8_t* message, size_t message_len, const uint8_t* rho, size_t rho_len, uint8_t* signature, size_t* signature_len) {
  if (!sk || !signature || !signature_len || *signature_len < FAEST_EM_192S_SIGNATURE_SIZE || (!message && message_len)) {
    return -1;
  }
...
int FAEST_CALLING_CONVENTION faest_em_192s_verify(const uint8_t* pk, const uint8_t* message, size_t message_len, const uint8_t* signature, size_t signature_len) {
  if (!pk || !signature || signature_len != FAEST_EM_192S_SIGNATURE_SIZE || (!message && message_len)) {
    return -1;
  }
...
void FAEST_CALLING_CONVENTION faest_em_192s_clear_unpacked_private_key(faest_em_192s_unpacked_private_key_t* key) {
