...
//      aes_sbox_comb.sv and aes_encipher_block.sv are reused verbatim from
//      secworks/aes -- both are already forward-only and keylen only
//      affects their internal round *count*, not decrypt logic, so no
//      changes needed there.
//
//  v4: uses aes_sbox_comb.sv (a private copy of the original combinational
//  aes_sbox), not aes_sbox.sv -- the latter became a configurable,
//  handshake-based module with three interchangeable backends, none of
//  which are 0-cycle combinational, and this core's own FSM (like
//  aes_encipher_block.sv/aes_key_mem_enc128.sv) was never touched to
//  handle that. See aes_sbox_comb.sv's header comment.
...
/* verilator lint_on UNOPTFLAT */
  ...
  aes_sbox_comb sbox_inst(.inv(1'b0), .sboxw(muxed_sboxw), .new_sboxw(new_sboxw));  //  forward-only: this core never decrypts
