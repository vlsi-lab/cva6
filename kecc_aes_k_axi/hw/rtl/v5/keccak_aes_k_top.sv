...
//  === v5: slice-serial Keccak (see keccak_slice_serial.sv's header
//      comment for the full derivation) -- the one shared 1600-bit
//      `state_reg` from Milestone 2 is gone. AES gets its own private
//      128-bit `aes_block_reg` back (it never needed 1600 bits; sharing
//      was about one FSM, not this register), and Keccak's whole
//      permutation state now lives in `keccak_slice_serial`'s internal
//      LUTRAM, processed PARALLEL_SLICES bits at a time instead of the
//      old keccak_round.sv's full-width-combinational one-cycle-per-round
//      (keccak_round.sv/keccak_round_constants_gen.sv are no longer
//      instantiated here -- keccak_slice_serial owns its own round-
//      constant generator internally). `keccak_din`/`keccak_dout` are
//      wired straight to the submodule's `state_in`/`state_out` (same
//      flat-bit lane convention, `64*(5*y+x)`, so no conversion needed).
//      CTRL_KECCAK_ROUND now just pulses `start` once and waits for
//      `done` instead of looping `round_ctr_reg` 0..23 itself.
//
//      Known limitation from this change: `zeroize` still aborts an
//      in-flight Keccak permutation immediately (wired into the
//      submodule's own reset) and still scrubs `aes_block_reg`, but it
//      no longer scrubs the *contents* of `keccak_slice_serial`'s LUTRAM
//      buffers within the same bounded 1 cycle -- doing that would need
//      extra clear cycles (writing 50 memories), contradicting zeroize's
//      "bounded at 1 cycle" guarantee. This matters because KMAC's key
//      is absorbed into the Keccak sponge state, so it's not purely
//      non-secret hash-only data passing through. Not fixed here --
//      flagged for a decision, not silently left broken; see
//      implementation.md's v5 entry.
...
module keccak_aes_k_top #(
                ...
                input  wire            clk,
                input  wire            reset_n,
                input  wire            zeroize,   //  scrub aes_block_reg + the AES key schedule to 0
                                                   //  (see v5 note above re: kslice's LUTRAM contents);
  ...
  // timing exactly), driven directly off kslice's own keccak_slice_done
  // pulse (see v5 note above) -- lands in lockstep with keccak_dout
  // becoming valid.
  reg         keccak_done_reg;
  reg         keccak_done_new;
  reg         keccak_done_we;

  // v5: AES's own private 128-bit working-block register -- no longer
  // shares storage with Keccak (see file header comment).
  ...
  reg          keccak_slice_start;   // pulse: keccak_slice_serial <= state_in=keccak_din, begin permutation
  wire         keccak_slice_done;    // pulse: keccak_slice_serial's state_out (-> keccak_dout) is valid

  wire [127 : 0] old_block = aes_block_reg;
  ...
  // v5: keccak_slice_serial's state_in/state_out use the exact same flat
  // bit convention (lane L=5y+x at bits [64L+63:64L]) as keccak_din/
  // keccak_dout already did, so no lane-remapping generate loop is
  // needed any more -- straight assign below (after the instance).
  ...
  keccak_slice_serial #(.PARALLEL_SLICES(PARALLEL_SLICES)) kslice(
                      .clk(clk),
                      .reset_n(reset_n && !zeroize),  // see file header comment
                      .start(keccak_slice_start),
                      .state_in(keccak_din),
                      .done(keccak_slice_done),
                      .state_out(keccak_dout)
                     );
  ...
  always @ (posedge clk or negedge reset_n)
    begin: reg_update
      if (!reset_n)
        ...
        begin
          // Same scrub as reset for AES's working block, but synchronous
          // and caller-triggered; core_ctrl_reg still moves to
          // CTRL_ZEROIZE via the normal core_ctrl_we/new path below (driven
          // by core_ctrl's own top-priority `if (zeroize)` check). Keccak's
          // in-flight state is aborted via kslice's own reset_n (see file
          // header comment on the LUTRAM-contents caveat).
          aes_block_reg <= '0;
        ...
        begin
          // v5: AES's own register now -- Keccak's state lives entirely
          // inside kslice (keccak_slice_serial), driven by keccak_slice_start/
          // keccak_slice_done instead of a shared register write here.
          if (muxed_w0_we)
            aes_block_reg[127 : 096] <= muxed_block_new[127 : 096];

          if (muxed_w1_we)
            aes_block_reg[095 : 064] <= muxed_block_new[095 : 064];

          if (muxed_w2_we)
            aes_block_reg[063 : 032] <= muxed_block_new[063 : 032];

          if (muxed_w3_we)
            aes_block_reg[031 : 000] <= muxed_block_new[031 : 000];
  ...
  always @*
    begin : core_ctrl
      ...
      case (core_ctrl_reg)
        ...
        CTRL_IDLE:
          begin
            if (init && (sel == SEL_AES))
              ...
              begin
                // v5: kslice owns its own internal round/slice counters --
                // this just pulses its start input once.
                keccak_slice_start = 1'b1;
                ready_new      = 1'b0;
                ready_we       = 1'b1;
        ...
        CTRL_KECCAK_ROUND:
          begin
            // v5: kslice runs its whole multi-cycle permutation internally
            // (see keccak_slice_serial.sv) -- just wait for its done pulse,
            // which lands in lockstep with kslice's state_out (->
            // keccak_dout) becoming valid.
            if (keccak_slice_done)
              begin
                ready_new       = 1'b1;
                ready_we        = 1'b1;
            ...
            // else: kslice is still running -- nothing else fires.
          end
